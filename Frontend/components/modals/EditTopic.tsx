...
function EditTopicModal({ topic, onClose, onTopicUpdated }: EditTopicModalProps) {
  const [formData, setFormData] = useState({
    ...
    isPublic: Boolean(topic.is_public)
  });
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 overflow-y-auto">
      <div className="relative w-full max-w-[600px] bg-white dark:bg-n800 rounded-2xl p-6 shadow-2xl animate-in fade-in zoom-in duration-300">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-medium">Edit Topic</h2>
          <button
            onClick={onClose}
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-12 flex items-center justify-between py-2">
            <div>
              <span className="text-sm font-medium block">Make Topic Public</span>
              <span className="text-xs text-n400 block">Public topics can be seen by other students</span>
            </div>
            <ToggleSwitch
